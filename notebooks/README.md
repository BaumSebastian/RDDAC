...
| [`07_preprocessing.ipynb`](07_preprocessing.ipynb) | Reference preprocessing via the `rddac preprocess` CLI: run on the small bundle, inspect the self-describing HDF5 attributes, raw-vs-processed oil trace, parameter variants with `--dump-config`/`--config`, and the pointcloud stage preview. |
