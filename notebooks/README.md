...
| [`07_preprocessing.ipynb`](07_preprocessing.ipynb) | Reference preprocessing via the `rddac preprocess` CLI: run on the small bundle, inspect the self-describing HDF5 attributes, the processing figures per modality (oil, force, sheet, luminescence) drawn from the package's own processing functions, parameter variants with `--dump-config`/`--config`, and the pointcloud stage preview. |
